...
L = np.linspace( 1000, 20000, 5000 ) * 1e-10  # м
L_min, L_max = L[0], L[-1]
W = c / L
W_min, W_max = W[-1], W[0]
...
cols = {'u': 'dodgerblue', 'g': 'g', 'r': 'r', 'i': 'goldenrod', 'z': 'k', 'y': '0.5',
        ...
        "u'": 'dodgerblue', "g'": 'g', "r'": 'r', "i'": 'goldenrod', "z'": 'k'}
...
vega_path = os.path.abspath("D:\\гаиш\\SLSN_2021\\alpha_lyr_stis_005.ascii")
...
def band_Plank_prime_T_2(w, b, T, z):
    wz = w + w*z / (z + 1)
    f = Band(w,b) * 2*h*wz**4 / (k*w*c**2) * np.exp( h*wz/(k*T) ) \
        * ( (np.exp( h*wz/(k*T) ) - 1)**2 * (-2*T - h*wz/k) + 
           2*(np.exp( h*wz/(k*T) ) - 1)*h*wz/k ) / T**4   \
            / (np.exp( h*wz/(k*T) ) - 1)**4
    return f
...
def plot_luminosity(slsn, z, data, save=0):
    ...
    if save == 1:
        ...
        fig.savefig( fname='D:\\гаиш\\SLSN_2021\\fix_x_bol_output\\figures\\blc\\' + 
                    slsn, bbox_inches="tight")
...
def plot_sub(slsn, z, data, save=0):
    ...
    if save == 1:
        #fig.savefig( fname='/media/documents/гаиш/SLSN_2021/bol_figures/' + 
                    #slsn, bbox_inches="tight")
        fig.savefig( fname='D:\\гаиш\\SLSN_2021\\bol_output_magn\\figures\\sub\\' + 
                    slsn, bbox_inches="tight")
        plt.close()
